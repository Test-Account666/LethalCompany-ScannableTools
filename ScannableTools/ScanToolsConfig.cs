...
public class ScanToolsConfig(ConfigFile configFile) {
    ...
    internal List<string> blacklistedItemsList = [];
    internal Regex? blacklistedItemsRegex;

    internal void HandleConfig() {
        keyScanNodeType = configFile.Bind<int>("Keys", "Key ScanNode Look", 0,
        ...
        blacklistedItems = configFile.Bind<string>("Items", "Blacklisted Items", "polaroid, test",
            "A comma separated list of items that will be ignored by this mod");

        blacklistedItems.SettingChanged += (_, _) => SetBlackListedItems();

        blacklistedItemsRegexEntry = configFile.Bind<string>("Items", "Blacklisted Items Regex", "",
